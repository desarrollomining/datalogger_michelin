document.addEventListener('DOMContentLoaded', () => {
            ...
            <div class="row g-4 justify-content-center">
                <div class="col-md-5">
                    <div class="card p-3 mb-3 shadow-sm border-start border-primary border-4">
                        <label for="input-vehiculo" class="form-label fw-bold text-primary">1. Nombre del Vehículo</label>
                        <div class="d-flex gap-2">
                            <input type="text" id="input-vehiculo" class="form-control" placeholder="Ej: CAEX-123" style="text-transform: uppercase;">
                            <button id="btn-asignar" class="btn btn-primary px-4 shadow-sm">
                                Asignar
                            </button>
                        </div>
                    ...
                    <div class="card p-4 mb-3 shadow-lg border-start border-warning border-4 bg-light">
                        <h5 class="fw-bold text-dark mb-2">3. Operación en Terreno</h5>
                        <p class="text-muted small mb-3">Presione el botón solo cuando el sensor esté posicionado correctamente en la rueda seleccionada.</p>
                        <button id="btn-escanear" class="btn btn-warning btn-lg w-100 py-3 fw-bold shadow text-uppercase border-2" style="font-size: 1.2rem; letter-spacing: 1px;">
                            Iniciar Escaneo
                        </button>
                    </div>
    ...
    async function iniciarEscaneo() {
        ...
        if (!vehiculoSeleccionado) {
            alert("Por favor, asigne o escriba un vehículo antes de iniciar el escaneo.");
            return;
        }

        btnEscanear.disabled = true;
        btnEscanear.className = "btn btn-secondary btn-lg w-100 py-3 fw-bold shadow text-uppercase border-2";
        btnEscanear.innerHTML = `<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> ESCANEANDO NEUMÁTICO [${posicionSeleccionada}]...`;
        ...
        try {
            ...
            if (data.status === "success") {
                ...
                alert(`Error en el escaneo: ${data.message}`);
            }
        } catch (error) {
            console.error("Error al conectar con la API de escaneo:", error);
            alert("No se pudo conectar con el servidor para iniciar el escaneo. Verifique la red.");
        } finally {
            btnEscanear.disabled = false;
            btnEscanear.className = "btn btn-warning btn-lg w-100 py-3 fw-bold shadow text-uppercase border-2";
